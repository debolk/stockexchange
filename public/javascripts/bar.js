$(document).ready(function(){
  ...
  $.getJSON('/commodities?110F4B0BDF366C453723', function(commodities) {
    $(commodities).each(function(){
      ...
      <div class="row"><input type="range" min="0" max="60" name="supply_price" value="' + this.supply_price + '"></div> \
      <div class="row"><input type="number" step="0.01" name="supply_rate" value="' + this.supply_rate + '"></div> \
      ...
      $('#update-' + this.name).on('click', function() {
        var update = {
          "supply_price":
            $('#commodity-' + name + ' input[name=supply_price]').val(),
          "supply_rate":
